...
update_names <- function(df, new_names=correct_names){
    df$school_name <- ifelse(df$school_name %in% correct_names$school_name, correct_names$financial_name[match(df$school_name, correct_names$school_name)], df$school_name)
    return(df)
}

update_names(df_1718)
update_names(df_1819)
update_names(df_1920)
update_names(df_2021)
update_names(df_2122)
