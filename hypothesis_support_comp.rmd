---
title: "support compensation hypo test"
author: "ks"
date: "12/3/2024"
...
# Scale 'support_comp' into spending groups (Low, High)
two_groups_df <- two_groups_df %>%
  mutate(support_comp_group = cut(
    support_comp,
    breaks = quantile(support_comp, probs = seq(0, 1, 0.5), na.rm = TRUE),
    labels = c("Low", "High"),
    include.lowest = TRUE
  ))

two_groups_df <- two_groups_df %>% filter(!is.na(support_comp_group))
head(two_groups_df)
```
